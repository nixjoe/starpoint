...
public class ResourceManager : MonoBehaviour {
    ...
    public StarpointResource Find(string name) {
        ...
        if (libraries.Exists(s => s.bundleName == bundle && s.name == library)) {
            SRL srl = libraries.Find(s => s.bundleName == bundle && s.name == library);
            if (srl.resourceList.Exists(r => r.name == resource)) {
                return srl.resourceList.Find(r => r.name == resource);
            } else {
