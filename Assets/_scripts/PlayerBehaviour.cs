...
public class PlayerBehaviour : Bolt.EntityBehaviour<IPlayerState> {
    ...
    private void LookUpdate(float roll, float pitch, float yaw) {
        ...
        if (Mathf.Abs(roll) >= 0.1f) {
            //if (!isBuilding) {
                rb.AddRelativeTorque(0f, 0f, roll * movementTorque / 10f, ForceMode.Force);
            //}
        } else {
        ...
        if (Mathf.Abs(mouseX) <= 0.1f) {
            ...
            if (mouseX > 0) {
                rb.AddRelativeTorque(0f, Mathf.Clamp(mouseX, 0, movementTorque), 0f, ForceMode.Force);
            } else {
                rb.AddRelativeTorque(0f, Mathf.Clamp(mouseX, -movementTorque, 0), 0f, ForceMode.Force);
            }
        }
        ...
        if (Mathf.Abs(mouseY) <= 0.1f) {
            ...
            if (mouseY > 0) {
                rb.AddRelativeTorque(Mathf.Clamp(mouseY, 0, movementTorque), 0f, 0f, ForceMode.Force);
            } else {
                rb.AddRelativeTorque(Mathf.Clamp(mouseY, -movementTorque, 0), 0f, 0f, ForceMode.Force);
            }
        }
    }
}
